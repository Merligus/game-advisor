...
Live-API status (as used by `recommender/enrichment.py`): **RAWG** is the reliable source for descriptions + covers (`background_image`, now mapped). **IGDB** gives portrait box-art covers + summaries; its `search` clause takes the **raw** string with `"`/`\` escaped — *not* URL-encoded (the old `quote()` call silently broke every multi-word/punctuated query). IGDB search is loose (returns bundles/spinoffs) so callers must name-match-guard its hits. **Gamespot** currently returns an HTML bot-challenge instead of XML — unusable live (the data was collected before that wall went up). **Metacritic/HLTB** carry no cover or description.
